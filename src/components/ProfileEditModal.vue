    ...
    <div @click.stop class="inline-block  bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg w-full">
        <div class="bg-white">
            ...
            <div class="flex flex-col p-2">
                ...
                <div class="mx-2 my-1 px-2 py-5 border text-gray-500 border-gray-200 reound hover:border-primary hover:text-primary">
                    <div class="text-sm"></div>
                    <input type="text" placeholder="자기소개"  class="text-black focus:outline-none">
                </div>
                <div class="mx-2 my-1 px-2 py-3 border text-gray-500 border-gray-200 reound hover:border-primary hover:text-primary">
                    <div class="text-sm"></div>
    ...
    setup(props, {emit}) {
        ...
        const onSaveProfile = async () => {
            ...
            if(backgroundImageData.value) {
                try {
                    const uploadTask = await storage.ref(`prifle/${currentUser.value.uid}/background`).put(backgroundImageData.value)
                    const url = await uploadTask.ref.getDownloadURL();
                    await USER_COLLECTION.doc(currentUser.value.uid).update({
                    ...
                    console.log(`background image data error:${error}`)
                }
            }
            emit('close-modal')
        }
        ...
        return {
            tweetBody, onAddTweet, currentUser, onChangeBackgroundImage, onChangeProfileImage, previewBackgroundImage, previewProfileImage, profileImage, backgroundImage, onSaveProfile
        }
